...
def run(catalog: str, schema: str, warehouse_id: str, app_sp_id: str) -> None:
    ...
    _sql(f"""
        CREATE TABLE IF NOT EXISTS {T("knowledge_graph")} (
            version     INT,
            created_at  TIMESTAMP,
            graph_json  STRING
        ) USING DELTA COMMENT 'Knowledge graph JSON — generated by the Knowledge Graph tab'
    """, warehouse_id, "Table knowledge_graph")
    ...
    grants = [
        ...
        (f"GRANT SELECT ON TABLE {T('knowledge_graph')} TO {SP}",             "knowledge_graph          SELECT"),
        (f"GRANT MODIFY  ON TABLE {T('knowledge_graph')} TO {SP}",            "knowledge_graph          MODIFY"),
    ]
