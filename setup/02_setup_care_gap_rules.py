...
# Ensure care_gap_findings table exists with the full schema.
# CREATE IF NOT EXISTS is idempotent — safe to run on every job execution.
spark.sql(f"""
...
# Add any columns missing from tables created with an older schema
...
print("care_gap_findings table ready")

# COMMAND ----------

# Grant app SP SELECT + MODIFY on care_gap_findings
if APP_SP_ID:
    for stmt in [
        ...
        try:
            ...
            print(f"  WARN: {stmt[:80]}… → {e}")
    print(f"Permissions granted on care_gap_findings to {APP_SP_ID}")
else:
