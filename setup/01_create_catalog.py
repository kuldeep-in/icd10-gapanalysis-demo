...
# MAGIC Creates the catalog, schemas, Delta tables, UC Volume, and care gap rules.
# MAGIC All statements use `IF NOT EXISTS` — safe to re-run.
...
spark.sql(f"CREATE SCHEMA IF NOT EXISTS `{CATALOG}`.clinical_data")
spark.sql(f"CREATE SCHEMA IF NOT EXISTS `{CATALOG}`.icd10_reference")
spark.sql(f"CREATE SCHEMA IF NOT EXISTS `{CATALOG}`.care_gaps")
spark.sql(f"CREATE SCHEMA IF NOT EXISTS `{CATALOG}`.app_config")
print("Schemas created")

# COMMAND ----------

# UC Volume for ICD-10 PDFs
spark.sql(f"CREATE VOLUME IF NOT EXISTS `{CATALOG}`.icd10_reference.pdfs")
print("Volume created: icd10_reference.pdfs")

# COMMAND ----------

# Clinical data tables
spark.sql(f"""
CREATE TABLE IF NOT EXISTS `{CATALOG}`.clinical_data.patient_records (
...
spark.sql(f"""
CREATE TABLE IF NOT EXISTS `{CATALOG}`.clinical_data.analysis_results (
    patient_id         STRING NOT NULL,
...
print("Clinical data tables created")

# COMMAND ----------

# Care gap tables
spark.sql(f"""
CREATE TABLE IF NOT EXISTS `{CATALOG}`.care_gaps.care_gap_rules (
...
spark.sql(f"""
CREATE TABLE IF NOT EXISTS `{CATALOG}`.care_gaps.care_gap_findings (
    patient_id         STRING,
    rule_id            STRING,
    gap_name           STRING,
...
print("Care gap tables created")

# COMMAND ----------

# Bootstrap status table
spark.sql(f"""
CREATE TABLE IF NOT EXISTS `{CATALOG}`.app_config.bootstrap_status (
...
""")
print("Bootstrap status table created")
...
existing_rules = spark.sql(f"SELECT COUNT(*) as cnt FROM `{CATALOG}`.care_gaps.care_gap_rules").collect()[0]["cnt"]

if existing_rules == 0:
    rules_df = spark.createDataFrame(
        [Row(rule_id=r[0], condition=r[1], gap_name=r[2], guideline=r[3], check_description=r[4], priority=r[5])
         for r in rules]
    )
    rules_df.write.mode("overwrite").saveAsTable(f"`{CATALOG}`.care_gaps.care_gap_rules")
    print(f"Inserted {len(rules)} care gap rules")
else:
...
WHEN MATCHED THEN UPDATE SET status = 'COMPLETED', updated_at = current_timestamp(),
    details = 'Catalog, schemas, tables, volume, and {len(rules)} care gap rules created'
WHEN NOT MATCHED THEN INSERT (step, status, updated_at, details)
    VALUES ('create_catalog', 'COMPLETED', current_timestamp(),
            'Catalog, schemas, tables, volume, and {len(rules)} care gap rules created')
""")
